...
class CoordinateVH(
    ...
    val minutes = if (minutes in 0u..59u) minutes
        else error("Minutes out of range: $minutes.\n Allowed values are between 0 and 59")
    val seconds = if (seconds in 0u..59u) seconds
    ...
    fun decimalStringify() : String {
        return "${((degrees.absoluteValue * 3600 + minutes.toInt() * 60 + seconds.toInt()) * degrees.sign).toFloat() / 3600}° $orientation)"
    }
